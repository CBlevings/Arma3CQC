...
if (isnull _unit) then {
	...
	switch _mode do {
		case "draw": {
			...
			if (shownmap) then {
				_ctrlList = _display displayctrl CQC_fnc_respawnMenuPosition_ctrlList;
...

				{
					...
					_colorLocal = if (_x in [CQC_fnc_respawnMenuPosition_selected,CQC_fnc_respawnMenuPosition_respawn]) then {[1,1,1,1]} else {_color};
				...
				} foreach CQC_fnc_respawnMenuPosition_positions;
			} else {
		...
		case "mousemoving": {
			...
			CQC_fnc_respawnMenuPosition_selected = _selected;
		};
		case "update": {
			_selected = _ctrlList lbdata lbcursel _ctrlList;
			if (_selected == "") then {_selected = missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_respawn",""];};
			...
			_respawnData = missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_data",[]];
			_respawnPositionsOld = missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_positions",[]];
			//if (count _respawnPositionsOld == 0) then {lbclear _ctrlList;};
			missionnamespace setvariable ["CQC_fnc_respawnMenuPosition_positions",+_respawnPositions];
			...
			for "_i" from 0 to (lbsize _ctrlList - 1) do {
				...
				if (_lbData != RANDOM_DATA && {{ alive _x && {side group _x != side group player} && {_x distance _pos <= 100} } count allUnits > 0}) then
				{
					...
					_ctrlList lbSetTooltip [_i, localize "STR_A3_CQC_fnc_respawnMenuPosition_warning"];
			...
			if (count _toAdd > 0) then {
				...
				missionnamespace setvariable ["CQC_fnc_respawnMenuPosition_data",+_respawnData];
			};
			...
			if (_setCurSel) then {
				...
				if (count _respawnPositions > 1) then {
					if (_ctrlList lbdata 0 != RANDOM_DATA) then {
						_lbAdd = _ctrlList lbAdd format ["<%1>",localize "STR_A3_CQC_fnc_respawnMenuPosition_random"];
				...
				if (isNil {missionNamespace getVariable "CQC_fnc_respawnMenuPosition_respawn"}) then {
					_cursel = 0;
				} else {
					for "_i" from 0 to (lbsize _ctrlList - 1) do {
						_lbData = _ctrlList lbdata _i;
				...
				[objnull,"select"] call CQC_fnc_respawnMenuPosition;
		...
		case "mousebuttondown": {
			...
			if (_button == 0) then {
				...
				_ctrlList = _display displayctrl CQC_fnc_respawnMenuPosition_ctrlList;
				for "_i" from 0 to (lbsize _ctrlList - 1) do {
					if (str CQC_fnc_respawnMenuPosition_selected == _ctrlList lbdata _i) then {
						CQC_fnc_respawnMenuPosition_systemSelect = true;
						CQC_fnc_respawnMenuPosition_respawn = CQC_fnc_respawnMenuPosition_selected;
		...
		case "select": {
			with missionnamespace do {
				if (lbsize _ctrlList == 0 || {missionNamespace getVariable ["CQC_fnc_respawnMenuPosition_systemSelect",false]}) exitwith {CQC_fnc_respawnMenuPosition_systemSelect = false;};
				_value = _ctrlList lbvalue lbcursel _ctrlList;
				_commitTime = missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_commitTime",0];
				if (_value != RANDOM_VALUE && lbcursel _ctrlList != -1) then {
					...
					CQC_fnc_respawnMenuPosition_respawn = _respawn;
				} else {
					//--- Random
					_respawnPositions = missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_positions",[]];
					...
					CQC_fnc_respawnMenuPosition_respawn = "";
				};
				CQC_fnc_respawnMenuPosition_commitTime = 0.2;
				_ctrlBlackContent ctrlsetfade 1;
				_ctrlBlackContent ctrlcommit 0.2;
			};
		};
	...
	if (!alive _unit) then {
		...
		[
			...
			{
				scriptname "CQC_fnc_respawnMenuPosition";
				_fnc_scriptName = "CQC_fnc_respawnMenuPosition";
				disableserialization;
				...
				switch _mode do {
					case "init": {
						...
						with missionnamespace do {
							CQC_fnc_respawnMenuPosition_ctrlList = ctrlIDC _ctrlList;
							CQC_fnc_respawnMenuPosition_selected = -1;
							CQC_fnc_respawnMenuPosition_positions = [];
							CQC_fnc_respawnMenuPosition_systemSelect = true;
							CQC_fnc_respawnMenuPosition_commitTime = 0;
						};

						_ctrlMap ctrladdeventhandler ["draw",{[objnull,"draw",_this] call CQC_fnc_respawnMenuPosition;}];
						_ctrlMap ctrladdeventhandler ["mouseMoving",{[objnull,"mousemoving",_this] call CQC_fnc_respawnMenuPosition;}];
						_ctrlMap ctrladdeventhandler ["mouseHolding",{[objnull,"mousemoving",_this] call CQC_fnc_respawnMenuPosition;}];
						_ctrlMap ctrladdeventhandler ["mouseButtonDown",{[objnull,"mousebuttondown",_this] call CQC_fnc_respawnMenuPosition;}];
						...
						_ctrlContent ctrlsetstructuredtext parsetext format [
							...
							(missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_text",toupper "No respawn positions available"])
						];
					...
					case "select": {
						if (lbcursel _ctrlList >= 0) then {[objnull,"select"] call CQC_fnc_respawnMenuPosition;};
					};
					case "update": {
						[objnull,"update"] call CQC_fnc_respawnMenuPosition;
					};
				};
			}
		...
		if (isplayer _unit) then {
			["CQC_fnc_respawnMenuPosition","RscDisplayLoadingBlack"] call bis_fnc_startloadingscreen;

			//--- Player - teleport to selected position
			[] call bis_fnc_showRespawnMenu;

			_respawn = missionnamespace getvariable ["CQC_fnc_respawnMenuPosition_respawn",""];
			...
			//--- Clean-up
			CQC_fnc_respawnMenuPosition_draw = nil;
			CQC_fnc_respawnMenuPosition_mouseMoving = nil;
			CQC_fnc_respawnMenuPosition_mouseButtonClick = nil;
			CQC_fnc_respawnMenuPosition_systemSelect = nil;
			CQC_fnc_respawnMenuPosition_positions = nil;
			CQC_fnc_respawnMenuPosition_commitTime = nil;
			with uinamespace do {
				CQC_fnc_respawnMenuPosition_ctrlList = nil;
				CQC_fnc_respawnMenuPosition_positions = nil
			};

			setplayerrespawntime _respawnDelay;

			"CQC_fnc_respawnMenuPosition" call bis_fnc_endloadingscreen;
		} else {
